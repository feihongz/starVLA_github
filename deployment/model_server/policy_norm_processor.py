...
used at training time from a checkpoint:
  ...
  1. Read ``config.full.yaml`` next to the ckpt when present, otherwise
     ``config.yaml`` → resolve ``data_mix`` →
...
class PolicyNormProcessor:
    ...
    Args:
        ckpt_path: Path to the ``*.pt`` checkpoint (the loader looks for
            ``config.full.yaml`` / ``config.yaml`` and
            ``dataset_statistics.json`` two dirs up).
