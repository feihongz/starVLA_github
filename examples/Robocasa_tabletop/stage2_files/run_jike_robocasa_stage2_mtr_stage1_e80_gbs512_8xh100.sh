...
RUN_ID="qwen_var_productvq_g16_s124816_robocasa_mtr_stage1_e80_100k_lr1e4_warmup5000_gbs512_jike8h100"
CONFIG_YAML="examples/Robocasa_tabletop/stage2_files/train_qwen_var_productvq_g16_s124816_robocasa_mtr_stage1_e80_100k_lr1e4_warmup5000_gbs512_jike8h100.yaml"
...
STAGE1_ARTIFACT="${STAGE1_DIR}/epoch_080.ckpt"
TOKEN_CACHE="${STAGE1_DIR}/stage2_token_cache_epoch080.pt"
...
# Keep the aborted pre-fix timing log intact for comparison.
...
EXPECTED_STAGE1_SHA256="87bbb33e8736bde8cdc8c62c0b7c604b00d17165ae84b8eb268454172d1a26db"
...
# Use the same Python environment as the verified 55.08% RoboCasa Stage2 run.
...
if [[ "${NUM_PROCESSES}" -ne 8 ]]; then
  echo "This launcher reproduces the 55.08% global-batch contract and requires NUM_PROCESSES=8; got ${NUM_PROCESSES}." >&2
  exit 1
fi
...
if [[ "${MIXED_PRECISION}" != "bf16" ]]; then
  echo "This launcher reproduces the 55.08% bf16 contract; got MIXED_PRECISION=${MIXED_PRECISION}." >&2
  exit 1
fi
...
cat <<EOF
[jike_robocasa_stage2_mtr_e80]
  baseline=55.08%@steps_86000 (661/1200 episodes)
  run_id=${RUN_ID}
  config=${CONFIG_YAML}
