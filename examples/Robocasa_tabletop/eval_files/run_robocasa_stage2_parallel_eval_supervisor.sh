...
launch_worker() {
  ...
  tmux new-session -d -s "${session}" \
    "cd ${STARVLA_DIR}; PYTHONPATH=${STARVLA_DIR} ${STARVLA_PYTHON} examples/Robocasa_tabletop/eval_files/run_robocasa_stage2_eval_chunked.py ${common_args[*]} --worker-index ${worker} --gpu ${gpu} >> ${launch_log} 2>&1"
}

summarize() {
  "${STARVLA_PYTHON}" examples/Robocasa_tabletop/eval_files/summarize_robocasa_success.py \
    "${OUTPUT_ROOT}" \
    "${extra_task_args[@]}" \
    --trials-per-task "${TRIALS_PER_TASK}" \
    --chunk-episodes "${CHUNK_EPISODES}" \
