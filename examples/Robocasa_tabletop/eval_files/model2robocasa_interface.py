...
class PolicyWarper:
    def __init__(
        ...
        self._step_count = 0
        self._action_stats_every = int(os.getenv("ROBOCASA_ACTION_STATS_EVERY", "0") or 0)
    ...
    def step(self, observations, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        self._step_count += 1
        if self._action_stats_every > 0 and self._step_count % self._action_stats_every == 0:
            finite = np.isfinite(raw_actions)
            print(
                "Action stats "
                f"step={self._step_count}, "
                f"shape={raw_actions.shape}, "
                f"finite={bool(finite.all())}, "
                f"min={float(np.nanmin(raw_actions)):.6f}, "
                f"max={float(np.nanmax(raw_actions)):.6f}, "
                f"mean={float(np.nanmean(raw_actions)):.6f}, "
                f"std={float(np.nanstd(raw_actions)):.6f}"
            )
    ...
    def visualize_epoch(
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "matplotlib is required only for visualize_epoch; install it or disable action plotting."
            ) from exc
