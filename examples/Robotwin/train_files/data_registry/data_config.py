...
_ROBOTWIN_TASK_NAMES = [name for name, _, _ in DATASET_NAMED_MIXTURES["robotwin"]]
DATASET_NAMED_MIXTURES.update(
    {
        "robotwin_clean_50": [(f"Clean/{name}", 1.0, "robotwin50") for name in _ROBOTWIN_TASK_NAMES],
        "robotwin_randomized_50": [(f"Randomized/{name}", 1.0, "robotwin50") for name in _ROBOTWIN_TASK_NAMES],
        "robotwin_clean_random_50": DATASET_NAMED_MIXTURES["robotwin_all_50"],
    }
)
