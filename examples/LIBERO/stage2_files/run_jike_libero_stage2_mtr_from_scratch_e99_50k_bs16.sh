...
RUN_ID="qwen_var_productvq_g16_s1248_mtr_from_scratch_e99_nextscale_50k_gbs128_jike8h100"
CONFIG_YAML="examples/LIBERO/stage2_files/train_qwen_var_productvq_g16_s1248_mtr_from_scratch_e99_nextscale_50k_gbs128_jike8h100.yaml"
...
BASE_MODEL="${BASE_MODEL:-/root/feihong/starVLA/playground/Pretrained_models/Qwen3-VL-4B-Instruct-VARAction}"
...
for required_path in "${CONFIG_YAML}" "${STAGE1_CONFIG}" "${STAGE1_ARTIFACT}" "${TOKEN_CACHE}" "${BASE_MODEL}/config.json"; do
...
PYTHON_BIN="/root/feihong/starVLA/.venv/bin/python"
if [[ ! -x "${PYTHON_BIN}" ]]; then
  echo "Python not found or not executable: ${PYTHON_BIN}" >&2
  exit 1
fi
"${PYTHON_BIN}" -c 'import transformers, accelerate, deepspeed, flash_attn, qwen_vl_utils; print(f"Stage2 dependency preflight OK: transformers={transformers.__version__}")'

export NUM_PROCESSES="${NUM_PROCESSES:-8}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
export MIXED_PRECISION="${MIXED_PRECISION:-bf16}"
export MAIN_PROCESS_PORT="${MAIN_PROCESS_PORT:-29567}"
...
[jike_libero_stage2_mtr_from_scratch]
  ...
  global_batch_size=$((NUM_PROCESSES * 16))
  max_train_steps=50000
  checkpoint_archive_range=24000-50000
