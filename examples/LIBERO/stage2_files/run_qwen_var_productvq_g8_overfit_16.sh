...
if [[ ! -f "${TOKEN_CACHE}" ]]; then
  echo "Missing ${TOKEN_CACHE}; build it first with examples/LIBERO/stage2_files/build_productvq_g8_full_token_cache.sh" >&2
  exit 1
fi
...
"${ACCELERATE_BIN}" launch \
  ...
  --config_yaml examples/LIBERO/stage2_files/train_qwen_var_productvq_g8_overfit_16.yaml
