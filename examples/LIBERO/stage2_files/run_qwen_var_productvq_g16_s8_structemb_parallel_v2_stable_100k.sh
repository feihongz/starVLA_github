...
if [[ ! -f "${TOKEN_CACHE}" ]]; then
  echo "Missing ${TOKEN_CACHE}; build it first with examples/LIBERO/stage2_files/build_productvq_g16_s8_structemb_full_token_cache.sh" >&2
  exit 1
fi
...
/home/zhangfeihong/miniconda3/envs/starVLA/bin/accelerate launch \
  ...
  --config_yaml examples/LIBERO/stage2_files/train_qwen_var_productvq_g16_s8_structemb_parallel_v2_stable_100k.yaml \
  2>&1 | tee "${LOG_FILE}"
