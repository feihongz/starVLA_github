...
CLIP_NORMALIZED_ACTIONS="${CLIP_NORMALIZED_ACTIONS}" \
...

    {
      ...
      date
      python examples/LIBERO/eval_files/summarize_libero_success.py "${MODEL_ROOT}/${EVAL_OUTPUT_ROOT}/logs" --chunked --require-ok-marker
      echo
    } | tee "${PROGRESS_PATH}"
  done
done
...
python examples/LIBERO/eval_files/summarize_libero_success.py "${MODEL_ROOT}/${EVAL_OUTPUT_ROOT}/logs" --chunked --require-ok-marker | tee "${SUMMARY_PATH}"
