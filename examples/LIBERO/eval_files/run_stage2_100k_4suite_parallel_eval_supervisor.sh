...
launch_suite() {
  ...
  tmux new-session -d -s "${session}" \
    "cd ${STARVLA_DIR}; USE_BF16=${EVAL_USE_BF16} SEED=${EVAL_SEED} TASK_SUITES_OVERRIDE=${suite} EVAL_OUTPUT_ROOT=${EVAL_OUTPUT_ROOT} TRIALS_PER_TASK=${TRIALS_PER_TASK} CHUNK_TRIALS=${suite_chunk} MAX_RETRIES=${MAX_RETRIES} CHUNK_TIMEOUT_SECONDS=${CHUNK_TIMEOUT_SECONDS} SAVE_VIDEOS=0 IMAGE_VIEWS=primary+wrist POLICY_IMAGE_SIZE=224 CONSTRAIN_TO_ACTION_TOKENS=0 CLIP_NORMALIZED_ACTIONS=0 VALIDATE_INPUTS=1 STRICT_TRIAL_COUNT=1 bash examples/LIBERO/eval_files/run_stage2_eval_chunked.sh ${CKPT} ${gpu} ${port} >> ${launch_log} 2>&1"
}
...
while true; do
  ...
  python examples/LIBERO/eval_files/summarize_libero_success.py "${LOG_ROOT}" --chunked --require-ok-marker > "${LOG_ROOT}/libero_40task_progress.txt" || true

  if [[ "${completed}" -eq "${#SUITES[@]}" ]]; then
    python examples/LIBERO/eval_files/summarize_libero_success.py "${LOG_ROOT}" --chunked --require-ok-marker | tee "${SUMMARY_PATH}"
