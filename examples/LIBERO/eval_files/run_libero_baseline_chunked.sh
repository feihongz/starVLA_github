...
for suite_idx in "${!TASK_SUITES[@]}"; do
  ...
  for task_id in $(seq 0 9); do
    ...
    done
    {
      ...
      date
      python examples/LIBERO/eval_files/summarize_libero_success.py "${MODEL_ROOT}/${EVAL_OUTPUT_ROOT}/logs" --chunked
      echo
    } | tee "${PROGRESS_PATH}"
  done
done
...
python examples/LIBERO/eval_files/summarize_libero_success.py "${MODEL_ROOT}/${EVAL_OUTPUT_ROOT}/logs" --chunked | tee "${SUMMARY_PATH}"
