...
launch_suite() {
  ...
  tmux new-session -d -s "${session}" \
    "cd ${STARVLA_DIR}; TASK_SUITES_OVERRIDE=${suite} TASK_START=${task_start} TASK_COUNT=${task_count} EVAL_OUTPUT_ROOT=${EVAL_OUTPUT_ROOT} TRIALS_PER_TASK=${TRIALS_PER_TASK} CHUNK_TRIALS=${suite_chunk} MAX_RETRIES=100000 CHUNK_TIMEOUT_SECONDS=1800 SAVE_VIDEOS=0 IMAGE_VIEWS=primary+wrist POLICY_IMAGE_SIZE=224 CONSTRAIN_TO_ACTION_TOKENS=0 CLIP_NORMALIZED_ACTIONS=0 bash examples/LIBERO/eval_files/run_stage2_eval_chunked.sh ${CKPT} ${gpu} ${port} >> ${launch_log} 2>&1"
}
...
while true; do
  ...
  python examples/LIBERO/eval_files/summarize_libero_success.py "${LOG_ROOT}" --chunked > "${LOG_ROOT}/libero_40task_progress.txt" || true

  if [[ "${completed}" -eq "${#SUITES[@]}" ]]; then
    python examples/LIBERO/eval_files/summarize_libero_success.py "${LOG_ROOT}" --chunked | tee "${SUMMARY_PATH}"
