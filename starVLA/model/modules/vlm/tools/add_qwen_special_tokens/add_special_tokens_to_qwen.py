...
def add_new_tokens(
    ...
    Returns:
      ...
      - action_token_start_idx: minimum target token id
      - action_token_end_idx: maximum target token id
    Notes:
    ...
    # 4) Target total size must cover tokenizer ids. Qwen models can ship with
    # reserved embedding rows, so newly added tokens may reuse rows that already
    # exist in the model embedding table.
    target_size = max(old_embed_size, len(tokenizer))
    new_token_ids = [int(tokenizer.convert_tokens_to_ids(t)) for t in to_add_tokens]
    all_token_ids = [int(tokenizer.convert_tokens_to_ids(t)) for t in new_tokens]

    # 5) If tokenizer total size exceeds model embedding size, resize first.
    action_token_start_idx = min(all_token_ids) if all_token_ids else old_embed_size
    action_token_end_idx = max(all_token_ids) if all_token_ids else old_embed_size - 1
...
def resolve_device_map(device: str):
    ...
    if device in {"cpu", "mps"}:
        return None
    raise ValueError(f"Unsupported --device {device!r}; use cuda, cuda:0, cpu, mps, or auto.")
...
def main():
    parser = argparse.ArgumentParser(description="Add special tokens to Qwen3-VL model and save to local directory.")
    parser.add_argument("--model-id", default="Qwen/Qwen3-VL-4B-Instruct", help="HF Hub model ID or local path")
    ...
    parser.add_argument(
        ...
        help="Attention implementation used while loading the model.",
    )
