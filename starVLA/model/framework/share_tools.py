...
def read_mode_config(pretrained_checkpoint):
    ...
    if os.path.isfile(pretrained_checkpoint):
        ...
        # Prefer the complete merged training config when available.  The
        # accessed-only config.yaml can omit fields used during data/prompt
        # construction, e.g. datasets.vla_data.CoT_prompt, which changes eval
        # behavior after loading from a checkpoint.
        ...
        # Load VLA Config (and corresponding base VLM `ModelConfig`) from `config.json`
        try:
            overwatch.info(f"Loading run config `{config_yaml}`")
