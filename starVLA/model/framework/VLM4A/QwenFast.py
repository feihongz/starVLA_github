...
class Qwenvl_Fast(baseframework):
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        constrain_to_action_tokens = bool(kwargs.get("constrain_to_action_tokens", False))
        max_new_tokens = kwargs.get("max_new_tokens", None)
        if max_new_tokens is None:
            max_new_tokens = 256 if constrain_to_action_tokens else None
        elif isinstance(max_new_tokens, str):
            max_new_tokens = int(max_new_tokens)

        generate_kwargs = dict(qwen_inputs)
        generate_kwargs["do_sample"] = bool(kwargs.get("do_sample", False))
        for generation_key in ("temperature", "top_p", "top_k", "num_beams"):
            if generation_key in kwargs and kwargs[generation_key] is not None:
                generate_kwargs[generation_key] = kwargs[generation_key]
        ...
        if constrain_to_action_tokens:
            ...
            allowed_tokens = list(range(act_min, act_max + 1))
            if eos_token_id is not None:
                allowed_tokens.append(eos_token_id)
            if pad_token_id is not None and pad_token_id != eos_token_id:
                allowed_tokens.append(pad_token_id)

            def _prefix_allowed_tokens_fn(batch_id, input_ids):
                return allowed_tokens

            generate_kwargs["prefix_allowed_tokens_fn"] = _prefix_allowed_tokens_fn
        ...
        prompt_action_token_ids = self._extract_action_token_ids(generated_ids[:, :input_token_len])
        generated_action_token_ids = self._extract_action_token_ids(generated_ids[:, input_token_len:])
        # --- Extract and decoder vlm_action to continue actions ---
        # --- extrace token (index based on VLM) ---
        batch_vlm_action_token_ids = generated_action_token_ids
        ...
        return {
            ...
            "action_tokens": batch_fast_action_token_idx,
            "generation_diagnostics": {
                ...
                "prompt_action_token_count": [
                    0 if seq is None else len(seq) for seq in prompt_action_token_ids
                ],
                "num_action_tokens": [0 if seq is None else len(seq) for seq in batch_fast_action_token_idx],
            },
        }
