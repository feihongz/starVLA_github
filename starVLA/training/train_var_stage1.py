...
def train(cfg: Any) -> None:
    ...
    libero10_records = [
        float(record["libero10_mae_mean"])
        for record in history
        if record.get("libero10_mae_mean", None) is not None
    ]
    best_libero10_mae = min(libero10_records) if libero10_records else float("inf")
    ...
    for epoch in range(start_epoch, int(cfg.train.epochs)):
        ...
        for scale in intermediate_scales:
            totals[f"scale_loss_s{scale}"] = 0.0
        batches = 0
        ...
        if reconstruction_stats is not None:
            ...
            libero10_task_records = [
                record
                for task_name, record in reconstruction_summary.get("tasks", {}).items()
                if str(task_name).startswith("libero_10::")
            ]
            if libero10_task_records:
                epoch_record["libero10_mae_mean"] = sum(
                    float(record["mae"]) for record in libero10_task_records
                ) / len(libero10_task_records)
        ...
        libero10_mae = epoch_record.get("libero10_mae_mean", None)
        if libero10_mae is not None and float(libero10_mae) < best_libero10_mae:
            best_libero10_mae = float(libero10_mae)
            save_checkpoint(
                output_dir / "best_libero10_mae.ckpt",
                model=model,
                optimizer=optimizer,
                epoch=epoch,
                history=history,
