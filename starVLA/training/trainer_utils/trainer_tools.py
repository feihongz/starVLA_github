...
def build_param_lr_groups(model, cfg):
    ...
    for module_name, lr in lr_cfg.items():
        if module_name == "base":
            continue
        # try to find the module/parameter under vla by module_name (support nested paths)
        module = model
        try:
            ...
            params = [p for p in module_params if p.requires_grad and id(p) not in frozen_params]
        ...
        if freeze_modules and type(freeze_modules) == str:
            ...
            for path in patterns:
                ...
                try:
                    for attr in attrs:
                        module = getattr(module, attr)
                    # if the module/parameter is successfully found, freeze it.
                    if isinstance(module, torch.nn.Parameter):
